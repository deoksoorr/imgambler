...
export default function BoardClient({ initialPosts, currentUserName, categorySlug, boardSlug, boardName, boardDescription }: any) {
  ...
  const { noticePosts, pinnedPosts, normalPosts } = useMemo(() => ({
    noticePosts: initialPosts.filter((p: any) => p.isNotice),
    pinnedPosts: initialPosts.filter((p: any) => p.isPinned && !p.isNotice),
    normalPosts: initialPosts.filter((p: any) => !p.isNotice && !p.isPinned)
  }), [initialPosts])
  ...
  // 페이지네이션 적용 (메모이제이션)
  const { totalPages, pagedNormalPosts } = useMemo(() => {
    ...
    return { totalPages, pagedNormalPosts }
  }, [filteredNormalPosts, currentPage, pageSize])
    ...
    <div className="w-full max-w-4xl mx-auto mt-8">
      <div className="mb-4">
        <div className="flex justify-between items-center">
          ...
          <Link href={`/board/${categorySlug}/${boardSlug}/write`} className="px-4 py-2 bg-blue-800 text-white rounded hover:bg-blue-900 transition font-bold border border-blue-900">Write Post</Link>
        </div>
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden mb-12 border border-gray-300">
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-100">
            <tr>
              <th className="px-4 py-3 text-left text-xs font-bold text-gray-800 uppercase min-w-[60px] max-w-[80px]">No.</th>
              <th className="px-4 py-3 text-left text-xs font-bold text-gray-800 uppercase w-full max-w-0">Title</th>
              <th className="px-4 py-3 text-left text-xs font-bold text-gray-800 uppercase min-w-[120px] max-w-[160px]">Author</th>
              <th className="px-4 py-3 text-left text-xs font-bold text-gray-800 uppercase min-w-[120px] max-w-[160px]">Date</th>
              <th className="px-4 py-3 text-left text-xs font-bold text-gray-800 uppercase">Views</th>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            ...
            {noticePosts.map((post: any) => (
              <tr key={post.id} className="bg-yellow-300 font-bold">
                <td className="px-4 py-3 text-center text-yellow-900"><FaCrown className="inline text-yellow-600 text-lg" title="Notice" /></td>
            ...
            {pinnedPosts.map((post: any) => (
              <tr key={post.id} className="bg-blue-50">
                <td className="px-4 py-3 text-center text-blue-700 font-bold"><FaThumbtack className="inline text-blue-700 text-lg" title="Pinned" /></td>
                <td className="px-4 py-3 w-full max-w-0">
